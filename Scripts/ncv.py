...
os.makedirs("output/ncv", exist_ok=True)
...
if args.save_embeddings==True:
        ...
        d2v_model.save("output/models/ncv_doc2vec_{0}.model".format(output))
        data.to_csv('output/models/ncv_{0}_shuffled_df_vectors.csv'.format(output), index=False)
else:
...
for i_its in range(inner_loop): # shuffle each tts of the 'data' set
    ...
    np.save('output/ncv/all_tts_aucs_{0}'.format(output), all_tts_aucs)
    np.save('output/ncv/all_permy_aucs_{0}'.format(output), all_tts_permy_aucs)
    np.save('output/ncv/all_fpr_tts_{0}'.format(output), all_tts_fprs)
    np.save('output/ncv/all_tpr_tts_{0}'.format(output), all_tts_tprs)
    np.save('output/ncv/all_best_thresh_{0}'.format(output), all_best_thresh)
    np.save('output/ncv/all_tpr_perm_aucs_{0}'.format(output), all_tts_permy_tprs)
    np.save('output/ncv/all_fpr_perm_aucs_{0}'.format(output), all_tts_permy_fprs)
    np.save('output/ncv/all_f1_scores_{0}'.format(output), all_tts_f1s)
    np.save('output/ncv/all_recalls_{0}'.format(output), all_tts_recalls)
    np.save('output/ncv/all_precisions_{0}'.format(output), all_tts_precisions)
    np.save('output/ncv/all_avg_precisions_{0}'.format(output), all_avg_precisions)
