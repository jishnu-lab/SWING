...
#import wandb 
# commandline args for the model 
...
handle = open("/ix/djishnu/Prabal/SWING/revision/D2V_PLM/embeddings_reproduction-master/src/embeddings_reproduction/mutint.embeds.9_4_2024.pkl",'rb')

embeds = pkl.load(handle)

handle.close()

# read in the tracker file 

data_tracker = pd.read_csv("mutint.prot.tracker.txt",sep="\t",header=None)

tracker = list(data_tracker[0])

ids2index = dict(zip(tracker,range(len(tracker))))
...
roc_fpr = os.path.join(path,name+".mutint.D2V-PLM.fpr.npy")
roc_tpr = os.path.join(path,name+".mutint.D2V-PLM.tpr.npy")
pr_prec = os.path.join(path,name+".mutint.D2V-PLM.precision.npy")
pr_recall = os.path.join(path,name+".mutint.D2V-PLM.recall.npy")
roc_auc = os.path.join(path,name+".mutint.D2V-PLM.auroc.npy")
f1_scores = os.path.join(path,name+".mutint.D2V-PLM.f1.npy")
